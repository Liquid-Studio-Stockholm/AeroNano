...
uint8_t minPressure = 5; //bar minimum pressure for misting
uint8_t maxPressure = 7; //bar
float curr_pressure = 0;
bool pressurize = false; // if true, pressure should be increased to maxPressure
long mockedPressureTimer;
int p_input = 0;

long mistingScheduleRunTime = 30;    //seconds to mist
long mistingSchedulePauseTime = 300; //seconds to pause
...
void loop()
{
   ...
   if (outputSensorData && (millis() - sensorOutputTimer > sensorOutputInterval))
   {
      printSensorData();
   }
}
...
void readPressure()
{
   ...
   p_input = analogRead(PRESSURE_SENSOR_PIN);
   curr_pressure = map(p_input, 110, 1023, 0, 13700);
   curr_pressure = curr_pressure / 1000;
}
...
void manageState()
{
   ...
   else if (state == STATE_MISTING)
   {
      ...
      /*
      Serial.print("Misting run time: ");
      Serial.print(state_run_time);
      Serial.print(" of ");
      Serial.print(mistingScheduleRunTime * 1000);
      */
...
*/
      if (state_run_time >= mistingScheduleRunTime * 1000)
      {
         ...
         state = STATE_PAUSE;
         state_time_left = 100; // 100% left at start
         state_start_time = millis();
   ...
   else if (state == STATE_PAUSE)
   {
      ...
      Serial.print(state_run_time);
      Serial.print(" of ");
      Serial.print(mistingSchedulePauseTime * 1000);
*/
      //percentage
...
*/
      if (state_run_time >= mistingSchedulePauseTime * 1000)
      {
         ...
         state = STATE_MISTING;
         state_time_left = 100; //100% left from the start
         state_start_time = millis();
         setRelayStateValue(MISTING_INDEX, RELAY_ON);
      }
   }
...
void managePressure()
{
   ...
   //turn on pump of misting and pressure is below min misting pressure
   if (relayStates[MISTING_INDEX] == RELAY_ON && curr_pressure <= minPressure)
   {
      pressurize = true;
   }
}
...
void processCommand()
{
   if (newCommand == true)
   {
      ...
      else if (receivedChars[0] == 'C') // Config value command
      {
         ...
         if (receivedChars[1] == 'P') // PRESSURE CONFIG
         {
